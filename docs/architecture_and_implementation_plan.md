...
状态：**已完成**。实现了显式配置与 seed 驱动的门、测量、syndrome、成像 loss 和并行 Rydberg 串扰采样；轨迹增加速度/间距元数据与交叉路径冲突组审计；ensemble 摘要、可视化噪声事件和 `d=5` 扩展测试均已加入。示例概率明确是 synthetic software parameters，并非设备标定。
...
5. M8 先以带来源标签的合成参数验证接口、复现性和扩展性；真实预测必须改用经过审计的实验参数。

在上述决策完成之前，不应开始大规模实现。M0 至 M8 已完成；后续工作应优先替换理想 decoder/backend，并用设备标定数据验证模型。
